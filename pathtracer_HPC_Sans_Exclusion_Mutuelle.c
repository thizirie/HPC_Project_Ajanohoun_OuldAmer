...
// pb du demandeur et du Isend 
int traiterMsg(long *const debut_fin, MPI_Status * const status, const int my_rank, const int p, bool *const a_linitiative , bool *const exit_bool, MPI_Request *const request){
	int demandeur = -1;
	switch((*status).MPI_TAG){
		case 1 : // qqun veut du travail 
			...
			if(demandeur == my_rank){
				...
				*a_linitiative = true;
			...
			else if((debut_fin[1] - debut_fin[0]) >= 2 ){ // jai du boulot, je lui envoie et je me met a jour et je met a jour ce que je dois faire ou retourner dans cette foncion
				...
				return 2; // j'ai envoye du travail 
			}
				...
				return 1; // j'ai relayé la demande de travail car j'en avais pas 
			}
...
{ 
	...
	double tps_comms = 0, debut_comm, fin_comm;
	double debut = wtime();
	...
	while(!exit_bool){
		...
		if(pix < debut_fin[1]){
			...
			if(flag == 1){
				traiterMsg(debut_fin, &status, my_rank, p, &a_linitiative, &exit_bool, &request );
			}
			fin_comm = wtime();

			tps_comms += fin_comm - debut_comm;
		}
		else if (!exit_bool){
			
			debut_comm = wtime();
			// Tag 1 : je veux du travail 
			///////////////////////////// gain avec ISend ? ?? 
			...
			bool travail_recu_ou_exit_bool = false;
			while(!travail_recu_ou_exit_bool){
				MPI_Probe(MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
				switch(traiterMsg(debut_fin, &status, my_rank, p, &a_linitiative, &exit_bool, &request )){
					case 6 : // travail recu !! 
						travail_recu_ou_exit_bool = true;
						break;

					case 5 : // je dois m'arreter !!
						travail_recu_ou_exit_bool = true;
						break;
